#!/usr/bin/env python3
# 2026-06-16 — H200 SM-scaling sweep for IQ-TREE3 OpenACC, ModelFinder = -m TEST
#
# One PBS job per SM count, REPEATS=1, same workload each time:
#     iqtree3 -s alignment_100000.phy -m TEST -ninit 2 -seed 1
#     (AA, LG+I+G4 data, 100 taxa, 100k sites)
# SM count restricted via CUDA MPS active-thread-percentage = SM/132*100.
#
# This run uses -m TEST, which tests only the standard rate models (no +R).
#
# Sweep points: 8, 16, 33, 66, 132 SMs. The 1/2/4-SM MPS requests were dropped
# this run because the 2026-06-13 sweep proved the H200 MPS minimum SM partition
# is ~8 SMs (1/2/4/8 all collapse to the same 8-SM allocation). So 8 IS the
# hardware floor and the speedup / efficiency baseline.
#
# Energy from IQ-TREE's Energy: block (CPU = RAPL, GPU = NVML).
#
# Data:  /Users/u7826985/Projects/Nvidia/results/2026_06_16_sm_sweep_m_test/sm_<N>/
# Out:   .../poc-gpu-likelihood-calculation_analysis/2026_06_16_sm_sweep_m_test/
...
NEW_ROOT = '/Users/u7826985/Projects/Nvidia/results/2026_06_16_sm_sweep_m_test'
OUT_DIR  = '/Users/u7826985/Projects/Nvidia/poc-gpu-likelihood-calculation_analysis/2026_06_16_sm_sweep_m_test'
TOTAL_SM = 132
FLOOR    = 8                     # H200 MPS minimum SM partition (proven 2026-06-13)
POINTS   = [8, 16, 33, 66, 132]

# ---- NVIDIA-ish palette + clean publication style (matches 2026_06_13_sm_sweep) ----
plt.rcParams.update({
...
G='#76B900'; GD='#4e7a00'; B='#0071C5'; O='#E8930C'; GREY='#9aa0a6'; RED='#C0392B'; INK='#202124'; PURP='#7A4FBF'
...
print('Parsing -m TEST run ...')
new = collect(NEW_ROOT, 'MTEST_sm_{sm}_rep1.log', 'MTEST_sm_{sm}_rep1_power.csv')

new.to_csv(os.path.join(OUT_DIR, 'sm_sweep_runs.csv'), index=False)
print('\n=== -m TEST summary ===')
print(new[['sm','pct','model','wall_s','mf_wall_s','ts_wall_s','speedup','efficiency',
           'gpu_J','cpu_J','measured_W','best_logL']].to_string(index=False))

new.to_csv(os.path.join(OUT_DIR, 'sm_sweep_summary.csv'), index=False)
X = new['sm'].values
...
WL = 'IQ-TREE3 OpenACC on H200  ·  -m TEST'

# 1. runtime
fig, ax = plt.subplots(figsize=(8,5.2))
ax.plot(X, new['wall_s']/60,'o-',color=G,lw=2.6,ms=9,zorder=5,label='Total wall')
ax.plot(X, new['ts_wall_s']/60,'s--',color=B,lw=1.8,ms=6,label='Tree search')
ax.plot(X, new['mf_wall_s']/60,'^--',color=O,lw=1.8,ms=6,label='ModelFinder (-m TEST)')
label_pts(ax, X, new['wall_s']/60, lambda v:f'{v:.0f}', dy=10)
...
# 2. GPU power
fig, ax = plt.subplots(figsize=(8,5.2))
ax.plot(X, new['measured_W'], 'o-', color=G, lw=2.4, ms=8, label='GPU board power (nvidia-smi sampled)')
ax.plot(X, new['gpu_W'], 'x--', color=GD, label='GPU avg (NVML energy / wall)')
label_pts(ax, X, new['measured_W'], lambda v:f'{v:.0f}W', dy=8, fs=9, color=GD)
logx(ax); ax.set_ylabel('Average GPU power (W)')
ax.set_title('GPU power vs SM  (rises with SMs → real partitions, no MPS floor here)\n'+WL); ax.legend()
fig.tight_layout(); fig.savefig(os.path.join(OUT_DIR,'fig_power_vs_sm.png')); plt.close(fig)

# 3. speedup by phase (with lost-scaling shading for total wall time)
fig, ax = plt.subplots(figsize=(8,5.2))
ax.fill_between(X, new['speedup'], new['ideal'], color=GREY, alpha=0.18, label='scaling lost to HBM bound')
ax.plot(X, new['ideal'],':',color=GREY,lw=2,label='ideal linear (from 8 SM)')
ax.plot(X, new['speedup'],'o-',color=G,lw=2.6,ms=9,zorder=5,label='Total wall time')
ax.plot(X, new['ts_speedup'],'s--',color=B,lw=1.9,ms=6,zorder=5,label='Tree search')
ax.plot(X, new['mf_speedup'],'^--',color=O,lw=1.9,ms=6,zorder=5,label='ModelFinder')
...
label_pts(ax, X, new['speedup'], lambda v:f'{v:.2f}×', dy=-16)
ax.annotate(f"{new['ts_speedup'].iloc[-1]:.2f}× tree search",
            (X[-1], new['ts_speedup'].iloc[-1]), textcoords='offset points',
            xytext=(8, 7), ha='left', fontsize=9, fontweight='bold', color=B)
ax.annotate(f"{new['mf_speedup'].iloc[-1]:.2f}× ModelFinder",
            (X[-1], new['mf_speedup'].iloc[-1]), textcoords='offset points',
...
ax.plot(X, new['efficiency']*100,'o-',color=G,lw=2.6,ms=9,zorder=5)
ax.axhline(100, ls=':', color=GREY, lw=1.5)
label_pts(ax, X, new['efficiency']*100, lambda v:f'{v:.0f}%', dy=10)
...
# 5. energy
fig, ax = plt.subplots(figsize=(8,5.2))
ax.plot(X, new['gpu_Wh'], 'o-', color=G, lw=2.4, ms=8, label='GPU (NVML)')
ax.plot(X, new['cpu_Wh'], 's--', color=B, label='CPU (RAPL)')
ax.plot(X, new['total_Wh'], '^-', color='black', lw=1.5, label='Total')
label_pts(ax, X, new['gpu_Wh'], lambda v:f'{v:.0f}', dy=9, fs=9, color=GD)
...
# 6. energy breakdown by phase/device
fig, ax = plt.subplots(figsize=(8.4,5.2))
xi=np.arange(len(X)); w=0.6
b1=new['mf_gpu_J']/3600; b2=new['ts_gpu_J']/3600; b3=new['mf_cpu_J']/3600; b4=new['ts_cpu_J']/3600
...
ax.plot(X, new['best_logL'].values, 'o-', color=G, lw=2)
ll = new['best_logL'].dropna().unique(); spread = (ll.max()-ll.min()) if len(ll) else 0.0
ax.set_ylabel('Best log-likelihood found'); logx(ax)
ax.set_title(f'Correctness — best logL vs SM (spread = {spread:.3g})\n'
             f'{"IDENTICAL across all SM counts → bit-exact" if spread==0 else "NOT identical"}; model = {new["model"].iloc[0]}')
fig.tight_layout(); fig.savefig(os.path.join(OUT_DIR,'fig_logL_agreement.png')); plt.close(fig)

# 8. EDP
fig, ax = plt.subplots(figsize=(8,5.2))
ax.plot(X, new['edp_Js']/1e9, 'o-', color=G, lw=2.4, ms=8)
best = new.loc[new['edp_Js'].idxmin()]
...
fig.suptitle('H200 SM scaling — IQ-TREE3 OpenACC, ModelFinder = -m TEST\n'
             '(8-SM hardware floor; AA LG+I+G4, 100 taxa × 100k sites; model picked = '
             f'{new["model"].iloc[0]}, logL bit-exact)', fontsize=14, fontweight='bold', y=0.985)
ax=axes[0,0]
ax.plot(X,new['wall_s']/60,'o-',color=G,lw=2.4,ms=8,label='Total wall')
ax.plot(X,new['ts_wall_s']/60,'s--',color=B,lw=1.6,ms=5,label='Tree search')
ax.plot(X,new['mf_wall_s']/60,'^--',color=O,lw=1.6,ms=5,label='ModelFinder')
label_pts(ax,X,new['wall_s']/60,lambda v:f'{v:.0f}',dy=9,fs=9)
logx(ax); ax.set_ylabel('Wall time (min)'); ax.set_title('(a) Runtime'); ax.legend(fontsize=8.5)
ax=axes[0,1]
ax.fill_between(X,new['speedup'],new['ideal'],color=GREY,alpha=0.18)
ax.plot(X,new['ideal'],':',color=GREY,lw=1.8,label='ideal linear')
ax.plot(X,new['speedup'],'o-',color=G,lw=2.4,ms=8,label='Total wall')
ax.plot(X,new['ts_speedup'],'s--',color=B,lw=1.6,ms=5,label='Tree search')
ax.plot(X,new['mf_speedup'],'^--',color=O,lw=1.6,ms=5,label='ModelFinder')
ax.set_xscale('log',base=2); ax.set_yscale('log',base=2)
ax.xaxis.set_major_locator(FixedLocator(X)); ax.xaxis.set_major_formatter(FixedFormatter([str(int(v)) for v in X]))
ax.yaxis.set_major_locator(FixedLocator([1,2,4,8,16])); ax.yaxis.set_major_formatter(FixedFormatter(['1×','2×','4×','8×','16×']))
ax.set_xlim(X.min()*0.82,X.max()*1.18); ax.set_xlabel('SMs used on H200  (hardware floor = 8)')
label_pts(ax,X,new['speedup'],lambda v:f'{v:.1f}×',dy=-15,fs=9)
ax.set_ylabel('Speedup vs 8-SM'); ax.set_title('(b) Speedup by phase'); ax.legend(fontsize=8.5,loc='upper left')
ax=axes[1,0]
ax.axhspan(80,112,color=G,alpha=0.08)
ax.plot(X,new['efficiency']*100,'o-',color=G,lw=2.4,ms=8)
ax.axhline(100,ls=':',color=GREY,lw=1.4)
label_pts(ax,X,new['efficiency']*100,lambda v:f'{v:.0f}%',dy=9,fs=9.5)
ax.set_ylim(40,112); logx(ax); ax.set_ylabel('Efficiency vs 8-SM (%)'); ax.set_title('(c) Parallel efficiency')
ax=axes[1,1]
xs=new['wall_s']/60; ys=new['gpu_Wh']
...
fig.text(0.5,0.005,f'Full GPU (132 SM) = {new["speedup"].iloc[-1]:.1f}× over the 8-SM floor, fastest AND lowest energy. '
         'H200 MPS min partition = 8 SM (proven 2026-06-13). logL identical across all runs.',
         ha='center',fontsize=9.5,style='italic')
fig.tight_layout(rect=[0,0.02,1,0.965])
fig.savefig(os.path.join(OUT_DIR,'fig_corrected_dashboard.png'))
fig.savefig(os.path.join(OUT_DIR,'fig_dashboard.png')); plt.close(fig)

# 10. compact runtime + speedup dashboard
fig, axes = plt.subplots(1,2, figsize=(13.5,5.2))
fig.suptitle('H200 SM scaling — IQ-TREE3 OpenACC, -m TEST, AA 100K',
