#!/usr/bin/env python3
"""
Analysis for R_v4 results: VANILA vs OPENACC across all models.
Generates:
  1. ll_comparison.csv — raw data (non-verbose only)
  ...
  5. Per-model +R4 comparison (styled like v3's detail plots)
  6. v3_vs_v4_R4_comparison.png — did the fix help?
...
results_v4 = "/Users/u7826985/Projects/Nvidia/results/2026_03_19_complexdata_bugfixes_R_v4"
...
def classify_backend(filename):
    """Classify backend from filename, skipping verbose files."""
...
def classify_backend_v3(filename):
    """Classify backend for v3 files (which may have VANILA_NONREV)."""
...
def parse_results(base_path, tag='', backend_classifier=None):
    """Parse all .iqtree results under base_path."""
    if backend_classifier is None:
        backend_classifier = classify_backend
    rows = []
...
# ============================================================
# 1. Parse v4 results (non-verbose only)
# ============================================================
print("Parsing v4 results (non-verbose)...")
df_v4 = parse_results(results_v4, tag='v4', backend_classifier=classify_backend)
print(f"  {len(df_v4)} rows")

# Also parse v3 for comparison
print("Parsing v3 results...")
df_v3 = parse_results(results_v3, tag='v3', backend_classifier=classify_backend_v3)
print(f"  {len(df_v3)} rows")

# ============================================================
# 2. Build comparison table (wide format) for v4
# ============================================================
...
wide_v4 = pivot_and_diff(df_v4, 'v4')
wide_v3 = pivot_and_diff(df_v3, 'v3')

# Save CSV
wide_v4.to_csv(os.path.join(out_dir, 'll_comparison.csv'), index=False)
print(f"Saved ll_comparison.csv ({len(wide_v4)} rows)")

# ============================================================
# 3. Correctness summary
# ============================================================
summary_rows = []
for (dt, topo, model), grp in wide_v4.groupby(['data_type','topology','model']):
    ...
    summary_rows.append({
        ...
        'status': 'PASS' if diffs.max() < 0.01 else ('WARN' if diffs.max() < 1.0 else 'FAIL'),
    })
...
make_correctness_heatmap(wide_v4, os.path.join(out_dir, 'correctness_matrix.png'), '(v4)')
...
for idx, dt in enumerate(['AA', 'DNA']):
    ax = axes[idx]
    subset = wide_v4[wide_v4['data_type'] == dt].dropna(subset=['ll_VANILA','ll_OPENACC'])
...
plt.suptitle('VANILA vs OPENACC Log-Likelihood (v4)', fontsize=14, fontweight='bold')
plt.tight_layout()
...
# ============================================================
# 6. V3 vs V4 comparison for +R4
# ============================================================
def make_v3_v4_comparison(save_path):
    """Compare R4 diffs between v3 and v4 to see if the fix helped."""
    ...
    for idx, (dt, topo) in enumerate([('AA','rooted'),('AA','unrooted'),
        ...
        r4_v4 = wide_v4[(wide_v4['data_type']==dt) & (wide_v4['topology']==topo) &
                          (wide_v4['model'].str.contains('R4'))].copy()

        if len(r4_v3) == 0 and len(r4_v4) == 0:
            ax.set_title(f'{dt}/{topo}/+R4 -- no data')
            continue

        for d, label, color in [(r4_v3, 'v3 (before fix)', COLORS['v3_OPENACC']),
                                 (r4_v4, 'v4 (after fix)', COLORS['v4_OPENACC'])]:
            if len(d) == 0:
                continue
            ...
            ax.bar(x + (0.2 if 'v4' in label else -0.2), diffs, 0.35,
    ...
    fig.suptitle('v3 vs v4: +R4 |VANILA - OPENACC| Improvement', fontsize=16, fontweight='bold')
    plt.tight_layout()
    plt.savefig(save_path, bbox_inches='tight')
    plt.close()
    print(f"Saved: {save_path}")

make_v3_v4_comparison(os.path.join(out_dir, 'v3_vs_v4_R4_comparison.png'))

# ============================================================
# 7. Per-combo R4 detail plots (styled like v3's detail plots)
# ============================================================
def make_r4_detail(data_type, topology, model, save_path):
    """Create detailed R4 comparison: VANILA vs OPENACC."""
    subset = wide_v4[(wide_v4['data_type']==data_type) & (wide_v4['topology']==topology) &
                      (wide_v4['model']==model)].copy()
    ...
    for _, r in subset.iterrows():
        if r['ll_diff'] >= 0.5:
            ax4.annotate(f"T{r['tree'].replace('tree_','')} (d={r['ll_diff']:.2f})",
                         (r['ll_VANILA'], r['ll_OPENACC']),
...
for idx, (dt, topo) in enumerate([('AA','rooted'),('AA','unrooted'),
                                   ('DNA','rooted'),('DNA','unrooted')]):
    ax = axes[idx//2][idx%2]
    subset = wide_v4[(wide_v4['data_type']==dt) & (wide_v4['topology']==topo)].copy()
    if len(subset) == 0:
        ax.set_title(f'{dt}/{topo} -- no data')
        continue
...
fig.suptitle('Max |VANILA - OPENACC| by Model (v4)', fontsize=16, fontweight='bold')
plt.tight_layout()
...
for dt in ['AA', 'DNA']:
    for topo in ['rooted', 'unrooted']:
        subset = wide_v4[(wide_v4['data_type']==dt) & (wide_v4['topology']==topo)]
        if len(subset) == 0:
            continue
        print(f"\n--- {dt} / {topo} ---")
        for model in sorted(subset['model'].unique()):
            ...
            n_exact = (diffs < 1e-4).sum()
            status = 'PASS' if diffs.max() < 0.01 else 'WARN' if diffs.max() < 1.0 else 'FAIL'
            print(f"  [{status:4s}] {model:12s}: max={diffs.max():.6f}  mean={diffs.mean():.6f}  exact={n_exact}/{len(diffs)}")

# V3 vs V4 comparison for R4
print("\n" + "=" * 100)
print("V3 vs V4 COMPARISON (+R4 only)")
print("=" * 100)
for dt, model in [('DNA','GTR+R4'), ('AA','LG+R4')]:
    for topo in ['rooted', 'unrooted']:
        r4_v3 = wide_v3[(wide_v3['data_type']==dt) & (wide_v3['topology']==topo) &
        ...
        v4_max = r4_v4['ll_diff'].max() if len(r4_v4) > 0 else float('nan')
        change = ''
        if pd.notna(v3_max) and pd.notna(v4_max):
            if v4_max < v3_max * 0.1:
                change = 'FIXED'
            elif v4_max < v3_max:
                change = 'improved'
            elif v4_max > v3_max:
                ...
                change = 'same'
        print(f"  {dt:3s}/{topo:9s}/{model:8s}: v3 max={v3_max:.6f}  v4 max={v4_max:.6f}  {change}")
